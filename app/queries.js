...
module.exports = {
  ...
  TIMERANGE: 'select MIN(?y) as ?min MAX(?y) as ?max  WHERE {?p a grz-owl:Person . ?p tis:hasTimeIndexedSetting ?s . ?s sem:hasTime ?t . ?t sem:hasTimeStamp ?d . BIND (year(?d) as ?y) }',

  generateSocialGraphQuery: function (minYear, maxYear) {
    return 'SELECT ?person ?plabel ?pgender ?connection ?clabel ?cgender ?ctype WHERE {?person a grz-owl:Person . ?person rdfs:label ?plabel . ?person grz-owl:gender ?pgender . ?person ?ctype ?connection . ?connection rdfs:label ?clabel . ?connection grz-owl:gender ?cgender . ?person tis:hasTimeIndexedSetting ?situation . ?situation sem:hasTime ?time . ?time sem:hasTimeStamp ?date . FILTER (year(?date) > ' + minYear + ' AND year(?date) < ' + maxYear + ')}';
  },
