...
module.exports = {
  ...
  generateContractsPerMonth: function (year) {
    return 'SELECT ?label (count(?contract) as ?count) WHERE {?contract a grz-owl:Contract . ?contract sem:hasTime ?temp . ?temp sem:hasTimeStamp ?date . BIND (year(?date) AS ?year) . FILTER (?year = ' + year + ') . BIND(month(?date) as ?label)} GROUP BY ?label'
  }
};
